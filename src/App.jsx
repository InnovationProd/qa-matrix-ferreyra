  ...
  if(page==='home')return(
    <div style={{minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'linear-gradient(165deg,#0F172A,#1E293B 50%,#0F172A)',padding:24}}>
      ...
      {linea&&<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:16,maxWidth:700,width:'100%'}}>
        ...
        {!result&&latestGiro&&<HC icon="📂" title="Último Giro" desc={`${latestGiro.name} · ${latestGiro.date}`} onClick={()=>loadGiro(latestGiro.id)}/>}
    ...
    <div style={{minHeight:'100vh'}}>
      ...
      <div style={{padding:'16px 24px',maxWidth:1900,margin:'0 auto'}}>
        <div className="fade-in" style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',gap:10,marginBottom:16}}>
          {[{l:'Registros',v:totalRecords,i:'📋'},{l:'Bancos',v:bancosControlados.toLocaleString(),i:'🏭'},{l:'Defectos',v:totalDefects,i:'🔍'},{l:'Tipos',v:totalDefectTypes,i:'📊'},{l:'AA',v:summary.AA,c:VC.AA,i:'🔴'},{l:'A',v:summary.A,c:VC.A,i:'🟠'},{l:'B',v:summary.B,c:VC.B,i:'🟡'},{l:'C',v:summary.C,c:VC.C,i:'🟢'}].map((k,i)=>(<div key={i} style={{background:'#1E293B',borderRadius:10,padding:'10px 12px',border:'1px solid #334155'}}><div style={{fontSize:10,color:'#94A3B8',marginBottom:3}}>{k.i} {k.l}</div><div style={{fontSize:22,fontWeight:700,color:k.c||'#F8FAFC',fontFamily:"'IBM Plex Mono'"}}>{k.v}</div></div>))}
        </div>
        ...
        <div style={{overflowX:'auto',borderRadius:10,border:'1px solid #334155'}}><table style={{width:'100%',borderCollapse:'collapse',minWidth:1400}}><thead><tr style={{background:'#1E293B'}}>
          ...
          {filteredRows.map((row,i)=>{const sel=selectedRow===row.vozNum;const pc=pdcaMap[row.vozNum]||{responsable:'',plan:false,do_step:false,check:false,act:false,comments:''};return[
            <tr key={row.vozNum} onClick={()=>setSelectedRow(sel?null:row.vozNum)} style={{background:sel?'#1E3A5F':(i%2===0?'#0F172A':'#131C2E'),cursor:'pointer',borderBottom:'1px solid #1E293B'}} onMouseEnter={e=>{if(!sel)e.currentTarget.style.background='#1E293B';}} onMouseLeave={e=>{if(!sel)e.currentTarget.style.background=i%2===0?'#0F172A':'#131C2E';}}>
              <td style={td}>{row.vozNum}</td><td style={td}><Voz v={row.voz}/></td><td style={{...td,textAlign:'left',fontWeight:500,fontSize:11}}>{row.concat}</td><td style={td}>{row.severidad}</td><td style={{...td,fontWeight:700}}>{row.cantDefectos}</td><td style={td}>{row.ocurrencia}</td><td style={td}>{row.detectabilidad}</td><td style={td}>{row.costo}</td><td style={{...td,fontWeight:700,color:'#F59E0B',fontSize:13,fontFamily:"'IBM Plex Mono'"}}>{row.index}</td>
