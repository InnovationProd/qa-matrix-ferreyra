...
import { fetchDefectos, upsertDefecto, deleteDefecto, bulkUpsertDefectos, saveGiro, fetchGiros, fetchGiro, deleteGiro, updateGiroRows, savePdca, fetchPdcas, saveUnificacion, fetchLineas, signIn, signOut, getSession, onAuthChange, subscribeGiros, subscribePdca, fetchScrapEventos, saveScrapEvento, deleteScrapEvento, subscribeScrap, fetchTiposAsiento, saveTipoAsiento, deleteTipoAsiento, fetchPartesAsiento, savePartesAsiento, deletePartesAsiento, fetchModelos, saveModelo, deleteModelo, fetchCuadrantes, saveCuadrante, deleteCuadrante, fetchReportesDefectos, fetchProduccionDiaria, upsertProduccionDiaria, fetchMncEventos, subscribeMnc } from './supabase';
...
export default function App(){
  ...
  if(page==='home')return(
    <div style={{minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'linear-gradient(165deg,#121212,#1F1F23 50%,#121212)',padding:24}}>
      ...
      <div style={{textAlign:'center',marginBottom:24}} className="fade-in"><div style={{fontSize:14,fontWeight:600,letterSpacing:4,color:'#B91C1C',textTransform:'uppercase',marginBottom:8}}>World Class Manufacturing</div><h1 style={{fontSize:36,fontWeight:700,color:'#FAFAFA',margin:0}}>Data Collector - Quality</h1></div>
    ...
    <div style={{minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'linear-gradient(165deg,#121212,#1F1F23 50%,#121212)',padding:24}}>
      ...
      {!pendingFile?(
        <div onDragOver={e=>{e.preventDefault();e.stopPropagation();setDragOver(true);}} onDragLeave={e=>{e.preventDefault();setDragOver(false);}} onDrop={e=>{e.preventDefault();setDragOver(false);handleFileDrop(e.dataTransfer?.files?.[0]);}} onClick={()=>fileRef.current?.click()} style={{width:'100%',maxWidth:520,border:`2px dashed ${dragOver?'#B91C1C':'#52525B'}`,borderRadius:16,padding:'48px 40px',textAlign:'center',cursor:'pointer',background:dragOver?'rgba(245,158,11,0.06)':'rgba(30,41,59,0.6)'}}>
          ...
          <div style={{fontSize:56,marginBottom:16}}>📊</div><p style={{fontSize:18,fontWeight:600,color:'#FAFAFA',margin:'0 0 8px'}}>Arrastrá el archivo Excel</p><p style={{fontSize:14,color:'#71717A',margin:'0 0 16px'}}>SurveyMonkey (.xlsx)</p><Btn bg="#27272A" color="#FAFAFA">Seleccionar archivo</Btn>
        </div>
      ):(
        <div style={{width:'100%',maxWidth:520,background:'rgba(30,41,59,0.8)',borderRadius:16,padding:32,border:'1px solid #3F3F46'}}>
          ...
          <label style={{display:'block',marginBottom:16}}><span style={{fontSize:12,fontWeight:600,color:'#B91C1C',textTransform:'uppercase',letterSpacing:1,display:'block',marginBottom:6}}>Bancos controlados *</span><input type="number" min="1" value={bancos} onChange={e=>setBancos(e.target.value)} placeholder="Ej: 5000" style={{width:'100%',padding:'10px 14px',borderRadius:8,border:'1px solid #52525B',background:'#1F1F23',color:'#FAFAFA',fontSize:16,fontWeight:700,fontFamily:"'IBM Plex Mono'"}}/><span style={{fontSize:11,color:'#71717A',marginTop:4,display:'block'}}>Piezas individuales controladas (usado para IPPM)</span></label>
              ...
              <div key={L} style={{background:'#1F1F23',borderRadius:14,padding:20,border:`2px solid ${LC[L]}`}}>
                ...
                <div style={{background:'#121212',borderRadius:10,padding:12,border:'1px dashed #52525B',marginBottom:16}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr auto',gap:6,alignItems:'end'}}>
                    ...
                    <Btn bg="#27272A" color="#FAFAFA" onClick={()=>handleSaveProduccionDiaria(L)} style={{padding:'6px 12px',fontSize:11}}>✓</Btn>
                  </div>
    ...
    <div style={{minHeight:'100vh',padding:24,maxWidth:1000,margin:'0 auto'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20,marginBottom:20}}>
        <div style={{background:'#1F1F23',borderRadius:12,padding:16,border:'1px solid #3F3F46'}}>
          ...
          <div style={{display:'flex',gap:8,marginBottom:12}}><input value={newTipoAsiento} onChange={e=>setNewTipoAsiento(e.target.value)} placeholder="Ej: Delantero" style={{flex:1,padding:'7px 10px',borderRadius:6,border:'1px solid #52525B',background:'#121212',color:'#FAFAFA',fontSize:13}}/><Btn bg="#27272A" color="#FAFAFA" onClick={handleAddTipoAsiento}>+</Btn></div>
        ...
        <div style={{background:'#1F1F23',borderRadius:12,padding:16,border:'1px solid #3F3F46'}}>
          ...
          <div style={{display:'flex',gap:8,marginBottom:12}}><input value={newModelo} onChange={e=>setNewModelo(e.target.value)} placeholder="Ej: Drive" style={{flex:1,padding:'7px 10px',borderRadius:6,border:'1px solid #52525B',background:'#121212',color:'#FAFAFA',fontSize:13}}/><Btn bg="#27272A" color="#FAFAFA" onClick={handleAddModelo}>+</Btn></div>
      ...
      <div style={{background:'#1F1F23',borderRadius:12,padding:16,border:'1px solid #3F3F46',marginBottom:20}}>
        ...
        <div style={{display:'flex',gap:8,marginBottom:12,flexWrap:'wrap'}}>
          ...
          <Btn bg="#27272A" color="#FAFAFA" onClick={handleAddParteAsiento}>+</Btn>
        </div>
      ...
      <div style={{background:'#1F1F23',borderRadius:12,padding:16,border:'1px solid #3F3F46'}}>
        ...
        <div style={{display:'flex',gap:8,marginBottom:12,flexWrap:'wrap'}}>
          ...
          <Btn bg="#27272A" color="#FAFAFA" onClick={handleAddCuadrante}>+</Btn>
        </div>
  ...
  if(page==='defectos'){const filtered=defSearch?defectos.filter(d=>d.nombre.toLowerCase().includes(defSearch.toLowerCase())):defectos;return(
    <div style={{minHeight:'100vh',padding:24,maxWidth:1100,margin:'0 auto'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24,flexWrap:'wrap',gap:12}}>
        ...
        <div style={{display:'flex',gap:8}}><Btn bg="#27272A" color="#FAFAFA" onClick={()=>setEditDef({nombre:'',severidad:3,costo_interno:1,costo_externo:4})}>+ Nuevo</Btn><Btn onClick={()=>defFileRef.current?.click()}>📤 Cargar Excel</Btn><Btn onClick={handleDownloadDefectos} bg="#D4D4D8" color="#FAFAFA">📥 Descargar</Btn><input ref={defFileRef} type="file" accept=".xlsx,.xls,.csv" onChange={e=>{if(e.target.files[0])handleUploadDefectos(e.target.files[0]);e.target.value='';}} style={{display:'none'}}/></div>
      </div>
      ...
      {editDef&&<div style={{background:'#1F1F23',borderRadius:12,padding:20,marginBottom:16,border:'1px solid #3F3F46'}}><div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr auto',gap:10,alignItems:'end'}}>
  ...
  if(page==='scrap')return(
    <div style={{minHeight:'100vh',padding:24,maxWidth:1400,margin:'0 auto'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:12}}>
        ...
        <Btn bg="#27272A" color="#FAFAFA" onClick={()=>openScrapForm({giroId})}>+ Registrar evento</Btn>
      </div>

      {scrapForm&&(
        <div style={{background:'#1F1F23',borderRadius:12,padding:20,marginBottom:20,border:'1px solid #3F3F46'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:12,marginBottom:14}}>
            <div><label style={{fontSize:11,color:'#B91C1C',display:'block',marginBottom:4}}>Componente *</label><select value={scrapForm.componente} onChange={e=>setScrapForm(p=>({...p,componente:e.target.value,defectoParte:''}))} style={{width:'100%',padding:'8px 10px',borderRadius:6,border:'1px solid #52525B',background:'#121212',color:'#FAFAFA',fontSize:13}}><option value="">Seleccionar...</option>{componentesUnicos.map(c=><option key={c} value={c}>{c}</option>)}</select>{componentesUnicos.length===0&&<span style={{fontSize:10,color:'#DC2626',display:'block',marginTop:3}}>Sin defectos cargados para {linea}</span>}</div>
            <div><label style={{fontSize:11,color:'#B91C1C',display:'block',marginBottom:4}}>Defecto *</label><select value={scrapForm.defectoParte} onChange={e=>setScrapForm(p=>({...p,defectoParte:e.target.value}))} disabled={!scrapForm.componente} style={{width:'100%',padding:'8px 10px',borderRadius:6,border:'1px solid #52525B',background:scrapForm.componente?'#121212':'#1F1F23',color:'#FAFAFA',fontSize:13,opacity:scrapForm.componente?1:0.5}}><option value="">Seleccionar...</option>{defectosPorComponente(scrapForm.componente).map(d=><option key={d.id} value={d.defectoParte}>{d.defectoParte}</option>)}</select></div>
            ...
            <div><label style={{fontSize:11,color:'#B91C1C',display:'block',marginBottom:4}}>Costo unitario (USD) *</label><input type="number" min="0" step="0.01" value={scrapForm.costoUnitario} onChange={e=>setScrapForm(p=>({...p,costoUnitario:e.target.value}))} placeholder="Ej: 12.50" style={{width:'100%',padding:'8px 10px',borderRadius:6,border:'1px solid #52525B',background:'#121212',color:'#FAFAFA',fontSize:13,fontFamily:"'IBM Plex Mono'"}}/></div>
          </div>
    ...
    <div style={{minHeight:'100vh'}}>
      <div className="print-header" style={{background:'linear-gradient(135deg,#1F1F23,#121212)',borderBottom:'1px solid #3F3F46',padding:'14px 24px',position:'sticky',top:0,zIndex:50}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12,maxWidth:1900,margin:'0 auto'}}>
          <div><div style={{fontSize:11,fontWeight:600,letterSpacing:3,color:'#B91C1C',textTransform:'uppercase'}}>WCM · Pilar Calidad · {linea}</div><h1 style={{fontSize:20,fontWeight:700,color:'#FAFAFA',margin:'2px 0 0'}}>{giroName||'Data Collector - Quality'}</h1></div>
          ...
          {filteredRows.map((row,i)=>{const sel=selectedRow===row.vozNum;const pc=pdcaMap[row.vozNum]||{responsable:'',plan:false,do_step:false,check:false,act:false,comments:''};const nodb=row.notInDb;return[
            ...
            sel&&<tr key={`d-${row.vozNum}`}><td colSpan={11+DETECTION_POINTS.length} style={{padding:'14px 16px',background:'#1F1F23',borderBottom:'2px solid #B91C1C'}}><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
              <div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(120px,1fr))',gap:8,marginBottom:12}}><Dt l="Componente" v={row.component}/><Dt l="Ocurrencia %" v={`${(row.ocurrenciaPct*100).toFixed(4)}%`} m/><Dt l="C.Int" v={row.costoInterno} m/><Dt l="C.Ext" v={row.costoExterno} m/><Dt l="C.Usado" v={row.costo} m h/><Dt l="Fórmula" v={`${row.severidad}×${row.ocurrencia}×${row.detectabilidad}×${row.costo}=${row.index}`} m h/></div>
                <div style={{marginTop:8,padding:'10px 12px',background:'#121212',borderRadius:8,border:'1px solid #3F3F46'}}><div style={{fontSize:10,color:'#B91C1C',fontWeight:600,textTransform:'uppercase',letterSpacing:1,marginBottom:6}}>Unificar voces</div><div style={{display:'flex',gap:8,alignItems:'center'}} onClick={e=>e.stopPropagation()}><input placeholder="Nros de voz (ej: 5,8,12)" value={unifyTarget?.vozNum===row.vozNum?unifyTarget.inputVal:''} onChange={e=>setUnifyTarget({vozNum:row.vozNum,inputVal:e.target.value})} style={{flex:1,padding:'6px 10px',borderRadius:6,border:'1px solid #52525B',background:'#1F1F23',color:'#FAFAFA',fontSize:12}}/><Btn bg="#27272A" color="#FAFAFA" onClick={()=>{if(unifyTarget?.vozNum===row.vozNum)handleUnify(row.vozNum,unifyTarget.inputVal);}} style={{padding:'6px 12px',fontSize:11}}>Unificar</Btn></div><p style={{fontSize:10,color:'#71717A',marginTop:4}}>Las ocurrencias se suman y las voces indicadas se eliminan</p></div>
